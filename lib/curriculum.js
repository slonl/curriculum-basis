var curriculum = module.exports = {};
	...
	curriculum.replace = function(section, id, newId, ...ids) 
	{
		...
		newObject.replaces   = newObject.replaces ? newObject.replaces.concat([id]) : [id];
			...
			curriculum.data[parentSection].filter(
				...
				function(entity) 
				{
					...
					if (entity.unreleased) {
						...
						newId = curriculum.add(section, newObject);
						curriculum.replace(section, entity.id, newId);
					}
				}
			);
		}
	...
	curriculum.getParentSections = function(section) 
	{
		...
		Object.keys(curriculum.data).forEach(
			...
			{
				if (typeof curriculum.data[dataSection][curriculum.getParentProperty(section)] != 'undefined') {
